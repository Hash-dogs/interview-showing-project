...
//对应三种不同的client
//    // 普通客户端：有 SkillsTool + Memory Advisor（有记忆）
//    createChatClient(id)
//
//    // Plain客户端：没有Tool，没有Memory（用于出题/评分，要纯JSON）
//    createPlainChatClient(id)
//
//    // Voice客户端：有SkillsTool，但手动管理历史，不加Memory Advisor
//    createVoiceChatClient(id)
    ...
    private ChatClient createChatClient(String providerId) {
        ...
        List<Advisor> advisors = buildDefaultAdvisors(providerId);//历史记录和memory包含在advisor中，集成了
        if (!advisors.isEmpty()) {
...
//            .streamToolCallResponses(streamToolCallResponses).build();
//
//    }

    private ToolCallAdvisor buildToolCallAdvisor(boolean conversationHistoryEnabled) {
        ToolCallAdvisor.Builder builder = (ToolCallAdvisor.Builder) ToolCallAdvisor.builder()
                .toolCallingManager(toolCallingManager)
                .advisorOrder(BaseAdvisor.HIGHEST_PRECEDENCE + 300);

        if (!conversationHistoryEnabled) {
            builder.disableInternalConversationHistory();
        }
