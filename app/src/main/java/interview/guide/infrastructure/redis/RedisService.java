...
public class RedisService {
    ...
    public <T> void set(String key, T value) {
        //redisson内的RBucket
        //getBucket（）没有实质访问redis只是创建了一个java类对象指向key值内容
    ...
    public String streamAdd(String streamKey, Map<String, String> message, int maxLen) {
        RStream<String, String> stream = redissonClient.getStream(streamKey, StringCodec.INSTANCE);
        StreamAddArgs<String, String> args = StreamAddArgs.entries(message);//等价于redis中的stream专用XADD命令
        if (maxLen > 0) {
            args.trimNonStrict().maxLen(maxLen);//采用模糊限定最大数，只要再限定树一定范围内即可
        }
