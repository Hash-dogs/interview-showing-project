...
public class ResumeGradingService {
    ...
    //下面两个是获取系统和用户提示词的工具类对象
    private final PromptTemplate systemPromptTemplate;
    private final PromptTemplate userPromptTemplate;
    //BeanOutputConverter是 Spring AI 框架中的一个工具类，用于将 LLM（大语言模型）的文本输出自动转换为 Java 对象。传入的DTO就是输出格式，能被其中相关逻辑用反射获取。
    private final BeanOutputConverter<ResumeAnalysisResponseDTO> outputConverter;
    private final StructuredOutputInvoker structuredOutputInvoker;
    
    // 中间DTO用于接收AI响应，在prompt中写明输出json的格式包含一下的字段
    ...
    public ResumeGradingService(
            ...
            ResumeAnalysisProperties properties,//用于写入提示词的路径
            ResourceLoader resourceLoader) throws IOException {
        this.llmProviderRegistry = llmProviderRegistry;
        this.structuredOutputInvoker = structuredOutputInvoker;
        this.systemPromptTemplate = new PromptTemplate(
            resourceLoader.getResource(properties.getSystemPromptPath())//提前在properties中写好prompt路径即可注入
    ...
    public ResumeAnalysisResponse analyzeResume(String resumeText) {
        ...
        try {
            ...
            // 拼接输出的json格式到系统提示词
            //其实system prompt中有json的输出格式，但是它依赖大模型去理解以及描述的清晰性。如果使用outputConverter.getFormat()可以无争议的输出相关格式的结果。
