...
public class KnowledgeBaseVectorService {
    ...
    public KnowledgeBaseVectorService(VectorStore vectorStore, VectorRepository vectorRepository) {
        ...
        // 使用 TokenTextSplitter（spring ai配置类方法） 默认配置，每个 chunk 约 800 tokens，基于标点边界切分（无重叠）
    ...
    /**
     * 基于多个知识库进行相似度搜索
     * 本质是给controller更好的传入参数调用vectorstore的同名函数similaritySearch()
     * @param query 查询文本
    ...
    /**
     * 向量搜索回退机制
     * 当基于元数据过滤的向量搜索失败时，使用此方法进行兜底检索
     * 策略：扩大检索范围（topK * 3），在内存中过滤知识库ID，最后截取 topK
     *
     * @param query 查询文本
     * @param knowledgeBaseIds 目标知识库ID列表，用于内存过滤
     * @param topK 期望返回的最大文档数量
     * @param minScore 最小相似度阈值
     * @return 过滤并截断后的相关文档列表
     */
    ...
    /**
     * 检查文档是否属于指定的知识库列表
     * 从文档元数据中提取 kb_id 并与目标列表比对
     *
     * @param doc 待检查的文档对象
     * @param knowledgeBaseIds 目标知识库ID列表
     * @return 如果文档属于指定知识库之一则返回 true，否则返回 false
     */
    ...
    /**
     * 构建知识库ID过滤表达式
     * 用于 Spring AI VectorStore 的 metadata 过滤查询
     * 格式示例: kb_id in ['1', '2', '3']
     *
     * @param knowledgeBaseIds 知识库ID列表
     * @return 符合 Spring AI 语法的过滤表达式字符串
     */
    private String buildKbFilterExpression(List<Long> knowledgeBaseIds) {
        ...
        return "kb_id in [" + values + "]";//返回筛选条件（[]是spring ai独特的语法格式）
    }
